...
tests: force ## Run all unit tests for Cilium
	$(MAKE) unit-tests
...
tags: $(GOLANG_SRCFILES) $(BPF_SRCFILES) cscope.files ## Generate tags for golang and bpf source files.
...
generate-hubble-api: api/v1/flow/flow.proto api/v1/peer/peer.proto api/v1/observer/observer.proto api/v1/relay/relay.proto ## Generate hubble proto golang sources.
	$(QUIET) $(MAKE) $(SUBMAKEOPTS) -C api/v1

generate-k8s-api: ## Generate Cilium k8s API client, deepcopy and deepequal golang sources.
...
gofmt: ## Run gofmt on golang source files in the repository.
	$(QUIET)for pkg in $(GOFILES); do $(GO) fmt $$pkg; done

govet: ## Run govet on golang source files in the repository.
...
logging-subsys-field: ## Validate logrus subsystem field for logs in golang source code.
...
update-dev-doctor-go-version: ## Update dev-doctor golang version.
...
update-gh-actions-go-version: ## Update GitHub action golang version.
...
update-travis-go-version: ## Update golang version in TravisCI config.
	$(QUIET) sed -i 's/go: ".*/go: "$(GO_VERSION)"/g' .travis.yml
	@echo "Updated go version in .travis.yml to $(GO_VERSION)"

update-test-go-version: ## Update golang version in test scripts.
...
update-images-go-version: ## Update golang version in Dockerfiles.
...
help: Makefile ## Display help for the Makefile, from https://suva.sh/posts/well-documented-makefiles/
